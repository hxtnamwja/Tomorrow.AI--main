        ...
        <div className="flex items-center justify-between p-6 border-b border-slate-200">
          <div>
            <h2 className="text-xl font-bold text-slate-800">
              {activeCommunity ? `${activeCommunity.name} - ` : ''}User Management
            </h2>
            <p className="text-sm text-slate-500 mt-1">
              {activeCommunity ? 'Manage users in this community' : 'Manage all platform users'}
            </p>
          </div>
          <button
            onClick={onClose}
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-slate-500">
                          {user.bio || 'No bio'}
                          {user.banReason && <span className="text-red-500 ml-2">• Reason: {user.banReason}</span>}
                        </p>
                      </div>
                    </div>
        ...
        {isBanModalOpen && selectedUser && (
          <div className="absolute inset-0 bg-black/30 flex items-center justify-center">
            <div className="bg-white rounded-2xl p-6 w-full max-w-md shadow-2xl">
              <h3 className="text-lg font-bold text-slate-800 mb-2">Ban User</h3>
              <p className="text-sm text-slate-500 mb-4">
                You are banning <span className="font-semibold">{selectedUser.username}</span>
              </p>
              <div className="mb-4">
                <label className="block text-xs font-bold text-slate-600 uppercase mb-1">Ban Reason (Optional)</label>
                <textarea
                  value={banReason}
